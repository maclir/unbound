 ...
 *
 */
var selectedRow, fixHeight, timer, filter;
var timeout = 1500;
(function ($) {
				...
				if (p.dataType == 'json') {
					...
					$("rows row", data).each(function () {
						...
						if (nid) {
							...
							if (tr.id == selectedRow) {
								tr.className = 'trSelected';
								if (status) {
									tr.className = 'trSelected ' + status.toLowerCase();
								}
							}
						}
						status = null;
			...
			populate: function () { //get latest data
				...
				$.ajax({
					...
					success: function (data) {
						$('.loading').hide();
						g.addData(data);
						sortGrid(".downloads-table", gridorder);
						timer = setTimeout("reloadGrid()", timeout);
					},
			...
			addRowProp: function () {
				$('tbody tr', g.bDiv).each(function () {
					$(this).click(function (e) {
						...
						if (selectedRow) enableButtons(); else disableButtons();
